...
// .wrangler/tmp/pages-oeoqjo/functionsWorker-0.11658047551081119.mjs
...
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    ).bind(body.platform, body.url, body.icon ?? "Mail", body.sort_order ?? 0).run();
...
var onRequestPut = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    await context.env.DB.prepare(
      ...
      WHERE id = ?`
    ).bind(body.platform ?? null, body.url ?? null, body.icon ?? null, body.sort_order ?? null, body.id).run();
  ...
  try {
    ...
    await context.env.DB.prepare(
      `UPDATE sections SET
        ...
        updated_at = datetime('now')
      WHERE id = ?`
    ).bind(body.title ?? null, body.subtitle ?? null, body.content ?? null, body.metadata ?? null, body.id).run();
    return Response.json({ success: true });
  } catch (e) {
    return Response.json({ error: String(e) }, { status: 500 });
  }
}, "onRequestPut");
function checkAuth3(request) {
  return !!request.headers.get("Authorization");
}
__name(checkAuth3, "checkAuth3");
__name2(checkAuth3, "checkAuth");
var onRequestPut4 = /* @__PURE__ */ __name2(async (context) => {
  if (!checkAuth3(context.request)) {
    return Response.json({ error: "Unauthorized" }, { status: 401 });
  }
  try {
    const body = await context.request.json();
    const stmts = Object.entries(body).map(
      ([key, value]) => context.env.DB.prepare(
...
var onRequest = /* @__PURE__ */ __name2(async (context) => {
  if (!context.env.DB && context.env.aminata_diop_db) {
    context.env.DB = context.env.aminata_diop_db;
  }
  return context.next();
}, "onRequest");
var routes = [
...
// .wrangler/tmp/bundle-feoLg6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-feoLg6/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.11658047551081119.js.map
